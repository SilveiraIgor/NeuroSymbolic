...
TOLERANCE = 20
nome_extra = "resumido_c5_logic_firstJ"
...
class MNISTSum2Dataset(torch.utils.data.Dataset):
  def __init__(
    ...
    if split == "train":
        self.essays = load_dataset("igorcs/LLM-C5-JBCS", cache_dir="tmp/aes_enem", trust_remote_code=True)['train']
        self.essays = self._normalizar(self.essays)
    elif split == "test":
        self.essays = load_dataset("igorcs/LLM-C5-JBCS", cache_dir="tmp/aes_enem", trust_remote_code=True)['test']
        self.essays = self._normalizar(self.essays)
        ...
        self.essays =  self.essays = load_dataset("igorcs/LLM-C5-JBCS", cache_dir="tmp/aes_enem", trust_remote_code=True)['validation']
        self.essays = self._normalizar(self.essays)
...
class MNISTNet(nn.Module):
  def __init__(self):
    ...
    self.problemas = AutoModelForSequenceClassification.from_pretrained(
                "neuralmind/bert-base-portuguese-cased",
                cache_dir="/tmp/aes_enem2",
                num_labels=2,
            )
    
    self.elementos = AutoModelForSequenceClassification.from_pretrained( 
                "neuralmind/bert-base-portuguese-cased",
                cache_dir="/tmp/aes_enem2",
                num_labels=6,
            )
  ...
  def forward(self, x):
    output1 = self.problemas(input_ids=x[0].to(device), token_type_ids=x[1].to(device), 
                        attention_mask=x[2].to(device))
    output2 = self.elementos(input_ids=x[0].to(device), token_type_ids=x[1].to(device), 
                        attention_mask=x[2].to(device))
    #print("Shapes: ", output1.logits.shape, output2.logits.shape)
...
class MNISTSum2Net(nn.Module):
  def __init__(self, provenance, k):
    ...
    self.scl_ctx.add_relation("problemas", int, input_mapping=list(range(2)))
    self.scl_ctx.add_relation("elementos", int, input_mapping=list(range(6)))
    #self.scl_ctx.add_relation("digit_2", int, input_mapping=list(range(10)))
    self.scl_ctx.add_rule("nota(0) :- problemas(1)")
    self.scl_ctx.add_rule("nota(1) :- elementos(0), problemas(0)")
    self.scl_ctx.add_rule("nota(1) :- elementos(1), problemas(0)")
    #soma2
    self.scl_ctx.add_rule("nota(2) :- elementos(2), problemas(0)")
    #soma3
    self.scl_ctx.add_rule("nota(3) :- elementos(3), problemas(0)")
    #soma4
    self.scl_ctx.add_rule("nota(4) :- elementos(4), problemas(0)")
    #soma5
    self.scl_ctx.add_rule("nota(5) :- elementos(5), problemas(0)")
  ...
  def forward(self, x: Tuple[torch.Tensor, torch.Tensor]):
    ...
    #print("Prints das respostas dos berts: ")
    #print(resposta_a, resposta_b)
    #print("Resposta da inferencia: ")
    #print(self.sum_2(problemas=resposta_a, elementos=resposta_b))#, digit_2=b_distrs) # Tensor 64 x 19
    return self.sum_2(problemas=resposta_a, elementos=resposta_b)#, digit_2=b_distrs) # Tensor 64 x 19
...
class Trainer():
  ...
  def test_whole_network(self, dataset_using, epoch, stage, num_test):
    ...
    with torch.no_grad():
      ...
      for (data, target) in iter:
        ...
        output = self.network(data).cpu()
        #print("Resposta depois, no test_whole: ", output)
        for vetor in output:
  ...
  def train(self, n_epochs):
    ...
    nome_arquivo = 'performances_c5_resumido_logic_'+nome_extra+'.csv'
