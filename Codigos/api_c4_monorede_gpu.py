...
TOLERANCE = 20
nome_extra = "resumido_monorede_c4_addmult_concatJ"
...
class MNISTSum2Dataset(torch.utils.data.Dataset):
  def __init__(
    ...
    if split == "train":
        self.essays = load_dataset("igorcs/LLM-C4-JBCS", cache_dir="tmp/aes_enem", trust_remote_code=True)['train']
        self.essays = self._normalizar(self.essays).shuffle(seed=42)
    elif split == "test":
        self.essays = load_dataset("igorcs/LLM-C4-JBCS", cache_dir="tmp/aes_enem", trust_remote_code=True)['test']
        ...
        self.essays =  self.essays = load_dataset("igorcs/LLM-C4-JBCS", cache_dir="tmp/aes_enem", trust_remote_code=True)['validation']
        self.essays = self._normalizar(self.essays)
...
class Trainer():
  ...
  def train(self, n_epochs):
    ...
    nome_arquivo = 'performances_c4_'+nome_extra+'.csv'
